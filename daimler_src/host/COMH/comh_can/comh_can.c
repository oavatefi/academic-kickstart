...
static CanWR_CHANNEL_ID_E CAN_CHANNEL;
...
static void InitCanWrapper(void);
...
#endif
/*CAN wrapper specific functions*/
...
u8 P2GPA_CanSendDebugCh (u16 id, const u8 *data, u8 dlc)
{
	...
	return CanWR_Tx(id, data, dlc, CAN_CHANNEL);

}

u8 P2GPA_CanSend (enum P2GPA_CAN_prio_E prio, u16 id, const u8 *p, u8 n)
{
	return CanWR_Tx(id, p, n, CAN_CHANNEL);
}
u8 P2GPA_CanSendExt (u32 id, const u8 *p, u8 n)
{
	return CanWR_TxExt(id, p, n, CAN_CHANNEL);
}
...
#endif
#ifdef FW_USE_LINUX_ICAM
	CAN_CHANNEL = CanWR_CHANNEL_ID_CAN0;
#else
	CAN_CHANNEL = CanWR_CHANNEL_ID_CAN1;
#endif
